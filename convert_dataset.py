...
def main(args)
    ...
    for impath in sorted(glob.glob(os.path.join('carla_data', '*.png'))):
        ...
        split = None
        if args.test_town in frame_name:
            split = 'test'
        else:
            split = 'train'
    ...
    for mapname in ['Town01', 'Town02', 'Town03', 'Town05']:
        ...
        train_cutoff = int(len(train_val_set)*args.val_split)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=float, 
        default=0.8,
        help="Portion of training dataset held for validation"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="Town05",
        help="Map to perform test-time evaluation in"
    )