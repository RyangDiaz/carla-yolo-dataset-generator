...
for data_type in ['images', 'labels']:
    for data_split in ['train', 'test']:
        os.makedirs(os.path.join('dataset', data_type, data_split), exist_ok=True)

for data_split in ['train', 'test']:
    os.makedirs(os.path.join('dataset', 'bounding_boxes', data_split), exist_ok=True)
...
for impath in sorted(glob.glob(os.path.join('carla_data', '*.png'))):
    ...
    shutil.copy(os.path.join('carla_data', f'{frame_name}.png'), os.path.join('dataset', 'images', split))
    shutil.copy(os.path.join('carla_data', f'{frame_name}.txt'), os.path.join('dataset', 'labels', split))
    shutil.copy(os.path.join('carla_data', 'boundingbox', f'{frame_name}_b.png'), os.path.join('dataset', 'bounding_boxes', split))
...
val_num = 0
test_num = 0
for mapname in ['Town01', 'Town02', 'Town03', 'Town04']:
    train_val_set = glob.glob(os.path.join('dataset', 'images', 'train', f'{mapname}*.png'))
    ...
    with open(os.path.join('dataset', 'train_yolo.txt'), "a") as f:
        for t in train_set:
            t = t.split('/')[-1]
            f.write(f"./images/train/{t}\n")
            train_num += 1
    
    with open(os.path.join('dataset', 'val_yolo.txt'), "a") as f:
        for t in val_set:
            t = t.split('/')[-1]
            f.write(f"./images/train/{t}\n")
            val_num += 1

test_set = sorted(glob.glob(os.path.join('dataset', 'images', 'test', '*.png')))
with open(os.path.join('dataset', 'test_yolo.txt'), "a") as f:
    for t in test_set:
        t = t.split('/')[-1]
